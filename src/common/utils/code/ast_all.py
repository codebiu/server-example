...
from sweepai.common.utils.comment_utils import check_comments_presence
...
"""
    ...
    min_line = 10
    max_line = 12
    path = code_tree.get_path_to_line(min_line=min_line, max_line=max_line)
     # 输出路径中的节点类型
    print("AST Path to lines", min_line, "to", max_line)
    for node in path:
        print(f"Type: {node.type}, Start: {node.start_point}, End: {node.end_point}")
        
    # 获取与指定行相关的代码块
    surrounding_lines = code_tree.get_lines_surrounding(min_line=min_line, max_line=max_line)
    print(f"Surrounding lines for lines {min_line}-{max_line}: {surrounding_lines}")

   
    # 测试词汇提取
    code_snippet = "def example_function(param1, param2): return param1 + param2"
    words = extract_words(code_snippet)
    print("Extracted words from snippet:")
    print(words)