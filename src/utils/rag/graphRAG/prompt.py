...
# promptType = {
#     "python": "专业的python软件开发助理",
# }

prompt_user = {
    "function": {
        "role": "user",
        "content": """你是一个专业的软件开发助理，熟悉多种编程语言和技术栈。
...
def get_prompt_user_function(code_langugae,code):
    prompt_result = {
        "role": "system",
        "content": f"""
                [[[代码语言:{code_langugae}
...
def get_prompt_user_class(describe):
    prompt_result = {
        "role": "system",
        "content": f"""
                [[[
                当前类内内容:{describe}]]]
                # 以上内容和数据是待分析内容!!!!!不要作为问题使用!!!!!!
                # 请用 Markdown 格式根据以上代码语言解析以上代码，并包含以下内容:
                    代码功能（用二级标题表示）。
                    代码mermaid
                # 输出Markdown格式:
...
if __name__ == "__main__":
    ...
    def simulate_chat():
        ...
        while True:
            ...
            # 获取当前的基础用户提示
            current_prompt = get_cur_base_user_prompt(message_history, index_prompt)
